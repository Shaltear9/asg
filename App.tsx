...
const App: React.FC = () => {
    ...
    const handleVideoUpload = async (file: File) => {
        ...
        try {
            ...
            // ★ 关键：三处都同步起来
            setVideoUrl(blob.url);          // 你原来就有的
            setVideoPreviewUrl(blob.url);   // 用于 <video> 播放
            setVideoBlobUrl(blob.url);      // 用于生成提示词时传给后端
        } catch (err) {
                        ...
                        <div className="mt-auto">
                            <button
                                ...
                                disabled={
                                    isUploadingVideo ||
                                    isLoading ||
                                    (!scriptText.trim() && !videoBlobUrl) // 用 videoBlobUrl 和上面保持一致
                                }
